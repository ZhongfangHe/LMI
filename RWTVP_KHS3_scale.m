...
%
% scale tau according to sqrt(tau) ~ C^+(0,1/sqrt(nK))


function draws = RWTVP_KHS3_scale(y, x, burnin, ndraws, ind_SV, ind_forecast)
% Inputs:
...
%                 tau, tau_d, tauj, tauj_d, phi, phi_d, rhoh, prior_rhoh);
    scale = 1/(n*K); %C^+(0,sqrt(scale))
    [tau, tau_d, tauj, tauj_d, phi, phi_d, rhoh] = KHS_update3_scale(diff_beta, diff_beta2,...
                tau, tau_d, tauj, tauj_d, phi, phi_d, rhoh, prior_rhoh, scale);            
    tmpmat = tau * (repmat(tauj',n,1) .* phi);
    for t = 1:n
        state_var{t} = diag(tmpmat(t,:));
    end %covar matrices of state noise for simulation smoother
